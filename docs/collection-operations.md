...
| --- | --- | --- |
| `full` | incremental daily OHLCV, TVL, CEX depth | manual catch-up and release validation |
| `daily` | incremental daily OHLCV, TVL | daily at 00:30 UTC |
| `tvl` | TVL only | manual retry/recovery |
| `depth` | CEX depth | hourly at minute 05 UTC |
...
- depth appends normalized history, then atomically replaces its latest
  snapshot.
...
and `cex_depth` separately. A global maximum date must not hide a lagging
source. Missing facts remain unavailable/null and are never replaced with zero.
