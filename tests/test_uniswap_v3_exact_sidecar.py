...
class UniswapV3ExactHealthTest(unittest.TestCase):
    ...
    def test_missing_configured_exact_csv_is_projected_without_http_failure(self):
        with patch.object(
            server,
            "resolve_dex_depth_path",
            side_effect=FileNotFoundError("configured depth is absent"),
        ):
        ...
        self.assertEqual(result["status"], "missing")
        self.assertEqual(result["depth_observed_count"], 0)
        self.assertEqual(result["depth_required_count"], 2)
        self.assertEqual(result["execution_required_scenario_count"], 20)
