...
def load_uniswap_v3_execution_authority(
    ...
    for index, raw_market in enumerate(markets):
        if not isinstance(raw_market, dict):
            raise ValueError(f"authority market {index} is not an object")
        missing = sorted(required - set(raw_market))
        if missing:
            raise ValueError(
                "Uniswap V3 execution authority is missing " + missing[0]
            )
        ...
        for field in (
            ...
            if not isinstance(value, str) or _EVM_ADDRESS.fullmatch(value) is None:
                raise ValueError(f"Uniswap V3 execution authority has bad {field}")
        if market["chain"] != "eth":
        ...
        if (
            len(parts) != 5
            or parts[:3] != ["dex", market["chain"], market["dex"]]
            or parts[3] != market["pool_address"]
            or not parts[4]
            or parts[4] != parts[4].upper()
        ):
            raise ValueError(
                "Uniswap V3 execution authority market identity mismatch"
            )
...
def validate_uniswap_v3_exact_public_receipt(
    ...
    *,
    ...
    if (
        ...
        or _ROUTE_BLOCK_HASH.fullmatch(block["hash"]) is None
    ):
...
def match_uniswap_v3_execution_authority(
    ...
    *,
    ...
    """Return the exact authority record or reject mismatched on-chain proof."""
    records = (
        dict(authority)
        if authority is not None
        else load_uniswap_v3_execution_authority()
    )
    ...
    for field, expected in pool_expected.items():
        actual = str(pool.get(field) or "")
        if field == "pool_address":
            actual = actual.lower()
        if actual != expected:
            raise ValueError(f"Uniswap V3 authority mismatch: {field}")
    expected_observed = {
        ...
        "token0_decimals": record["token0_decimals"],
        "token1_address": record["token1_address"],
        "token1_decimals": record["token1_decimals"],
        "fee_pips": record["fee_pips"],
        "tick_spacing": record["tick_spacing"],
    }
    for field, expected in expected_observed.items():
        if field not in observed_identity:
            raise ValueError(f"Uniswap V3 authority evidence is missing {field}")
...
def exact_v3_block_identity(
    ...
    block_hash = str(block.get("hash") or "").lower()
    if _ROUTE_BLOCK_HASH.fullmatch(block_hash) is None:
...
def collect_dex_pool_observation(
    ...
    *,
    ...
    try:
        ...
        expected_v3_block_identity: dict[str, str] | None = None
        if block_number is None:
            if exact_v3_approved:
                ...
                expected_v3_block_identity = exact_v3_block_identity(
                    finalized_block,
                    block_number,
                )
            else:
            ...
            if fixed_block_header is not None:
                expected_v3_block_identity = exact_v3_block_identity(
                    fixed_block_header,
                    block_number,
                )
            ...
            if block_number == finalized_number:
                ...
                if (
                    expected_v3_block_identity is not None
                    and expected_v3_block_identity != finalized_identity
                ):
...
def collect_dex_depth_with_execution(
    ...
    *,
    ...
    block_headers: dict[str, Mapping[str, Any]] = {}
    finalized_chains = {
        pool["chain"].lower()
        for pool in pools
        if protocol_model(
    ...
    for index, pool in enumerate(pools, start=1):
        ...
        if block_number is None and chain in finalized_chains:
            ...
            exact_v3_block_identity(finalized_header, block_number)
            blocks[chain] = block_number
            block_timestamps[chain] = block_timestamp
            block_headers[chain] = finalized_header
        row, pool_execution_rows = collect_dex_pool_observation(
            pool,
            snapshot_id=snapshot_id,
            raw_path=raw_path,
            ...
            fixed_block_header=block_headers.get(chain),
        )
...
def validate_snapshot(
    ...
    *,
    ...
    for row in rows:
        ...
        if status in {"observed", "partial"} and (
            is_uniswap_v3_execution_approved(row)
        ):
            require_v3_usd_price_lineage(
                snapshot_id=row.get("usd_price_source_snapshot_id"),
                source=row.get("usd_price_source"),
                endpoint=row.get("usd_price_source_endpoint"),
                raw_sha256=row.get("usd_price_raw_response_sha256"),
            )
...
def normalized_execution_gate_rows(
    ...
    """Do not exclude a declared strict adapter that returned unsupported."""
    normalized = []
    for source_row in rows:
        row = dict(source_row)
        if row.get("status") == "unsupported":
            ...
            if model == "constant_product_v2" or (
                model == "concentrated_liquidity_v3"
                and is_uniswap_v3_execution_approved(row)
            ):
                row["status"] = "failed"
        normalized.append(row)
    return normalized
...
def publish_full_publication_bundle(
    ...
    *,
    ...
    exact_validation_receipt: Optional[Mapping[str, Any]] = None,
    authority_path: Path = V3_EXECUTION_AUTHORITY_PATH,
