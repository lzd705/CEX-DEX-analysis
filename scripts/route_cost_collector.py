...
try:
    from scripts.bounded_json import (
        BoundedJsonError,
        _bounded_json_shape as _shared_bounded_json_shape,
        _preflight_json_bytes as _shared_preflight_json_bytes,
        decode_bounded_json_response,
    )
except ModuleNotFoundError:  # pragma: no cover - direct script execution
    from bounded_json import (  # type: ignore
        BoundedJsonError,
        _bounded_json_shape as _shared_bounded_json_shape,
        _preflight_json_bytes as _shared_preflight_json_bytes,
        decode_bounded_json_response,
...
_BOUNDED_JSON_ROUTE_MESSAGES = {
    "limit_invalid": "route-cost wire limit is invalid",
    "headers_invalid": "route-cost response headers are invalid",
    "headers_ambiguous": "route-cost response headers are ambiguous",
    "resource_limit": "route-cost wire resource limit exceeded",
    "content_length_invalid": "route-cost Content-Length is invalid",
    "content_length_mismatch": (
        "route-cost Content-Length differs from wire bytes"
    ),
    "encoding_unsupported": "route-cost response encoding is unsupported",
    "deadline_stream_invalid": (
        "route-cost deadline-bound stream is invalid"
    ),
    "monotonic_invalid": (
        "route-cost deadline monotonic capability is invalid"
    ),
    "deadline": "route-cost response deadline exceeded",
    "stream_unavailable": "route-cost response stream is unavailable",
    "stream_invalid": "route-cost response stream is invalid",
    "gzip_invalid": "route-cost gzip response is invalid",
    "json_invalid": "route-cost JSON response is invalid",
    "duplicate_keys": "route-cost JSON has duplicate keys",
    "number_invalid": "route-cost JSON number is invalid",
    "object_key_invalid": "route-cost JSON object key is invalid",
    "string_invalid": "route-cost JSON string is invalid",
    "value_invalid": "route-cost JSON value is invalid",
    "noncanonical": "route-cost JSON response is noncanonical",
}


def _bounded_json_route_error(error: BoundedJsonError) -> RouteCostCollectorError:
    message = _BOUNDED_JSON_ROUTE_MESSAGES.get(error.detail_code)
    if message is None:
        message = "route-cost JSON response is invalid"
    return RouteCostCollectorError(message)
...
def _bounded_json_shape(
    ...
    try:
        _shared_bounded_json_shape(
            value,
            node_limit=node_limit,
            ordinary_string_limit=ordinary_string_limit,
            permit_binary_float=permit_binary_float,
        )
    except BoundedJsonError as error:
        raise _bounded_json_route_error(error) from None
...
def _decode_bounded_json_response(
    ...
    try:
        return decode_bounded_json_response(
            ...
            materialize_exact_floats=materialize_exact_floats,
            absolute_deadline=absolute_deadline,
            monotonic=time.monotonic,
            return_decoded_bytes=return_decoded_bytes,
        )
    except BoundedJsonError as error:
        raise _bounded_json_route_error(error) from None
